...
These decouple streaming *state machines* (event handlers/routers) from
*side-effects* (message persistence, logging, analytics, ...). Event handlers
...
event handler is present on the router.
...
from unique_toolkit._internal.streaming.common import ActivityStatus
...
class StreamStarted:
    ...
    Attributes:
        ...
        content_chunks: Retrieved chunks available to subscribers for
            ...
            text event handlers never need to know about them.
    """
...
class TextUpdate:
    ...
    subscribers can persist incremental updates without poking at event handler
    internals.
    """
...
@dataclass(frozen=True, slots=True)
class StreamEnded:
    """Published once after every event handler has flushed and the stream is closed.

    Carries the final accumulated text for the authoritative persist, any
    ``appendices`` contributed by auxiliary event handlers (e.g. a code interpreter
    block or tool-activity summary), and final-message metadata owned by the
    orchestrator. Subscribers that write the assistant message are expected to
    concatenate ``appendices`` onto ``full_text`` in order, avoiding a second
...
class StreamEventBus:
    ...
    entirely when the router has no progress-producing event handler).
...
class StreamSubscriber(Protocol):
    ...
    replaces passing a fan-out ``EventHandler[StreamEvent]`` callable: it keeps
