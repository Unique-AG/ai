    ...
    class FakeRequestor(EndpointRequestorProtocol):
        ...
        def request(
            ...
            **kwargs: CombinedParamsSpec.kwargs,
            ...
            # path_params = self._endpoint.path_params_model(*args, **kwargs)
            # url = self._endpoint.create_url_from_model(path_params)

            # payload_model = self._endpoint.payload_model(*args, **kwargs)
            # payload = self._endpoint.create_payload_from_model(payload_model)

            # response = requests.request(
            #    method=self._endpoint.request_method(),
            #    url=url,
            #    headers=headers,
            #    json=payload,
            # )
