...
user_confirmation_tool_description = LanguageModelToolDescription(
    ...
    description="This tool is used to present the information (json) by the user. It will show the user and ask them for confirmation. This should be used to whenever you want to do an API call and you want to ensure the user has confirmed the content of the API call.",
...
def human_verification_tool_call_handler(
    ...
    # If the first call to the LLM fails to produce the ticket data
    try:
        model_data = HumanResourcesTicketData.model_validate(**data_to_present)
    except Exception:
        completion = client.beta.chat.completions.parse(
            model=model,
            messages=builder.messages,
            response_format=HumanResourcesTicketData,
        )
    ...
    chat_service.create_assistant_message(
        ...
        f"[Please Confirm the above data by clicking here and sending the data in your next message](https://prompt={urllib.parse.quote(f'I confirm the data below \n\n\n --- \n  {json.dumps(api_call_request.model_dump(), indent=2)}')})",
