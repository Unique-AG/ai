...
class AsyncExecutor:
    ...
    def __init__(self, logger: Optional[logging.Logger] = None, context_manager: Optional[Callable[[], AsyncContextManager]] = None) -> None:
    ...
    async def run_async_tasks(
        ...
        async def logging_wrapper(task: Awaitable[T], task_id: int) -> Result:
            ...
            self.logger.info(f"Thread {thread.name} (ID: {thread.ident}) starting task {task_id}")
            ...
            try:
                ...
                self.logger.error(f"Thread {thread.name} (ID: {thread.ident}) - Task {task_id} failed with error: {e}")
                return e
            finally:
                end_time = time.time()
    ...
    async def run_async_tasks_in_threads(
        ...
        def thread_worker(task_chunk: list[Awaitable[T]], chunk_id: int) -> list[Result]:
            thread = threading.current_thread()
            self.logger.info(f"Thread {thread.name} (ID: {thread.ident}) starting chunk {chunk_id} with {len(task_chunk)} tasks")
            ...
            try:
                ...
                end_time = time.time()
                duration = end_time - start_time
                self.logger.info(f"Thread {thread.name} (ID: {thread.ident}) finished chunk {chunk_id} in {duration:.2f} seconds")
                return results
            except Exception as e:
                self.logger.error(f"Thread {thread.name} (ID: {thread.ident}) encountered an error in chunk {chunk_id}: {str(e)}")
                raise
            finally:
                loop.close()
        ...
        self.logger.info(f"Splitting {len(tasks)} tasks into {len(task_chunks)} chunks across {max_threads} threads")
        ...
        end_time = time.time()
        duration = end_time - start_time
        self.logger.info(f"All threads completed. Total results: {len(results)}. Duration: {duration:.2f} seconds")
