...
NORMALIZATION_PATTERNS: list[NormalizationPattern] = [
    *_STRIP_NON_SOURCE_PATTERNS,
    ...
    (
        r"(?i)(?<!\[)source[\s_]?(\d+)",
        r"<sup>\1</sup>",
    ),  # source0, SOURCE0, source_0 not preceded by [
...
BATCH_NORMALIZATION_PATTERNS: list[NormalizationPattern] = [
    *_STRIP_NON_SOURCE_PATTERNS,
    ...
    (r"(?i)(?<!\[)source[\s_]?(\d+)", r"[\1]"),  # source0, SOURCE0, source_0 → [0]
...
def extract_cited_sequence_numbers(text: str) -> set[int]:
    """Return the 1-based source sequence numbers cited in ``<sup>`` tags."""
    return {int(match) for match in _CITED_SUP_PATTERN.findall(text)}


def chunks_to_sdk_references(chunks: list[ContentChunk]) -> list[Message.Reference]:
    """Convert chunks to SDK reference dictionaries using their 1-based list position."""
    return [chunk.to_sdk_reference(i) for i, chunk in enumerate(chunks)]


def filter_cited_sdk_references(
    chunks: list[ContentChunk],
    text: str,
) -> list[Message.Reference]:
    """Build SDK references for ``chunks``, keeping only those cited in ``text``.

    A chunk at 0-based index ``i`` is cited iff ``<sup>i+1</sup>`` appears in
    ``text``. If ``chunks`` is empty, returns an empty list.
    """
    if not chunks:
        return []
    cited = extract_cited_sequence_numbers(text)
    if not cited:
        return []
...
class StreamingPatternReplacer:
    ...
    def process(self, delta: str) -> str:
        ...
        if self._max_match_length == 0:
            released = self._apply_replacements(self._buffer)
            self._buffer = ""
            return released
