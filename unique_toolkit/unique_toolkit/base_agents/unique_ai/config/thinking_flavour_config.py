...
from _common.stock_ticker import StockTickerConfig, StockTickerDetectionConfig
...
from _common.tools.web_search.services.search_engine.bing import (
    BingConfig,
    BingCustomSearchConfig,
)
from unique_ai.config import SearchAgentConfig, UploadedContentConfig
...
if __name__ == "__main__":
    ...
    websearch_tool = ToolBuildConfig(
        ...
        configuration=WebSearchConfig(
            ...
            search_engine_config=BingConfig(
                custom_search_config=BingCustomSearchConfig(
    ...
    config = SearchAgentConfig(
        ...
        token_limits=LoopAgentTokenLimitsConfig(
            language_model=big_llm,
            percent_of_max_tokens_for_history=0.4,
        ),
    ...
    with Path.open(
        write_folderpath / "unique-ai-thinking-flavour.json", "w"
    ) as f:
        f.write(
            config.model_dump_json(
                by_alias=True,
                indent=4,
                exclude=set(["language_model_info"]),
            ),
        )
