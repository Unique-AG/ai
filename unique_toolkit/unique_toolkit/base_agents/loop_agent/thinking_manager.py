...
from unique_toolkit.unique_toolkit.language_model.schemas import LanguageModelAssistantMessage, LanguageModelStreamResponse
...
class ThinkingManagerConfig(BaseModel):
  thinking_steps_display: bool  = Field(default=True, description="Whether to display thinking steps in the chat.")
...
class ThinkingManager():
    ...
    def update_start_text(self, start_text: str, loop_response: LanguageModelStreamResponse) -> str :
    ...
    def close_thinking_steps(self,loop_response: LanguageModelStreamResponse):
        ...
        loop_response.message.text = (
            loop_response.message.text.replace(
