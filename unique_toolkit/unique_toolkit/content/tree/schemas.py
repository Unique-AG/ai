...
- :class:`FuzzyMatch` is the result record returned by
  :meth:`unique_toolkit.content.tree.service.ContentTree.search_visible_files_fuzzy_async`.

Kept free of behaviour so they can be imported and introspected without pulling
in the SDK/HTTP stack used by :mod:`unique_toolkit.content.tree.functions`.
"""
    ...
    def format_trie_walk(
        ...
        *,
        ...
        if max_depth is not None and depth >= max_depth:
            hidden_dirs = sum(1 for _ in self.children)
            hidden_files = len(self.files)
