...
:mod:`unique_toolkit.content.tree.functions` so it can be constructed and used
on its own.
"""
...
class ContentTree:
    ...
    :mod:`unique_toolkit.content.tree.functions`. It takes the same identity
        ...
        *,
        ...
        for content_info, segments in rows:
            ...
            key_score = (
                ...
                if match_on in ("key", "both")
                else 0.0
            )
            path_score = (
                difflib.SequenceMatcher(None, normalized_query, path_candidate).ratio()
                if match_on in ("path", "both")
                else 0.0
            )

            if key_score >= path_score:
