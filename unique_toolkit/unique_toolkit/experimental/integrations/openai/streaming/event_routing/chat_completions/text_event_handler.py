"""EventHandler for Chat Completion text deltas — accumulates text via replacers.

This event handler is a *pure state machine*: it applies streaming replacers and
...
event on its own event-handler-owned :class:`TypedEventBus` so external
subscribers (typically the orchestrator, optionally tests or tracers) can
react without the event handler needing to know about the outer
...
class ChatCompletionTextEventHandler:
    ...
    def text_bus(self) -> TypedEventBus[TextFlushed]:
        """Event-handler-local bus carrying :class:`TextFlushed` at every flush."""
    ...
    def get_usage(self) -> LanguageModelTokenUsage | None:
        """Return token usage from the terminal chunk, when OpenAI provides it."""
        return self._usage
