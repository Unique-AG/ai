...
from unique_toolkit.language_model.constants import DEFAULT_COMPLETE_TEMPERATURE
...
def _is_response_output_items(
    ...
    """Narrow a message sequence to raw Responses output models."""
    return len(msgs) > 0 and all(
...
def _build_default_router() -> ResponsesStreamEventRouter:
    ...
    event handler wired with the shared citation-normalization replacer, plus
    the standard tool-call, completed, and code-interpreter event handlers.
    All event handlers are pure — side-effects live in the default subscribers
    ...
    return ResponsesStreamEventRouter(
        text_event_handler=ResponsesTextDeltaEventHandler(replacers=replacers),
...
class ResponsesCompleteWithReferences(ResponsesSupportCompleteWithReferences):
    """``ResponsesSupportCompleteWithReferences`` backed by the event handler router.

    Wiring mirrors :class:`ChatCompletionsCompleteWithReferences`:

    * Event handlers/router accumulate state only (pure), including the code
      interpreter event handler which publishes progress updates only (no executed
      code appended to the saved message).
    ...
    * **Settings-driven** (``settings`` only, optionally ``router`` /
      ...
      standard tool-call, completed and code-interpreter event handlers); the
    ...
    * **Instance injection** (``settings`` + explicit ``client``,
    ...
    def __init__(
        ...
        *,
        ...
        # Per-request context for the event-handler bus adapters. Set at the top
    ...
    def _default_subscribers(self) -> list[StreamSubscriber]:
        ...
        has a progress-producing event handler (i.e. ``activity_bus`` exists);
        ...
        from event handler presence to subscriber set.
        """
    ...
    async def _on_activity_progress_update(
        ...
        """Adapter: lift an event-handler bus :class:`ActivityProgressUpdate` to outer :class:`ActivityProgress`.

        Attaches ``message_id`` / ``chat_id`` — event handlers stay ignorant of
        bus-level identifiers, which means a future progress-producing
        event handler just needs to publish :class:`ActivityProgressUpdate` on
    ...
    async def complete_with_references_async(  # noqa: PLR0913
        ...
        *,
        ...
        # ``start_text`` is on the
        # :class:`ResponsesSupportCompleteWithReferences` protocol but is
        ...
        def input_messages(
            ...
            if _is_language_model_messages(message_items):
                return _convert_messages_to_openai(message_items)

            if _is_response_input_items(message_items):
                return message_items

            if _is_response_output_items(message_items):
                return _convert_messages_to_openai(message_items)
            ...
            raise TypeError(
                ...
                "homogeneous list of ResponseInputItemParam dicts, or a "
                "homogeneous list of ResponseOutputItem models; got a "
