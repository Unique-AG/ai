...
def get_full_history_with_contents(
    ...
    for c in grouped_elements:
        ...
        if text is None:
            ...
            text = ""
        # NOTE: empty assistant messages (text == "") are passed through here
        # unchanged. This can happen when a turn was cancelled before the LLM
        # produced a final prose response. The resulting empty ASSISTANT message
        # is benign at the end of history but worth reviewing if it causes
        # unexpected LLM behaviour in practice.
