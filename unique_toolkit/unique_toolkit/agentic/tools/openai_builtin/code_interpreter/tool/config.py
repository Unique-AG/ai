...
_TOOL_DESCRIPTION_FILES_SUFFIX = """
...
{%- if failed_uploads and failed_uploads | length > 0 %}

## Files That Could NOT Be Uploaded
The following files were referenced by the user or the system but could NOT be
made available in this environment. They do NOT exist at any path — do not
attempt to open, read, or reference them, and do NOT say the file is
"missing" or "not found" without explanation. Instead, clearly tell the user
which file failed and why, using the reason given below:
...
_TOOL_DESCRIPTION_FOR_SYSTEM_PROMPT_BASE = """
...
_TOOL_DESCRIPTION_FOR_SYSTEM_PROMPT_FENCE_BASE = """
Use this tool to run python code, e.g to generate plots, process excel files, perform calculations, etc.
...
- HTML files MUST follow these best practices so they render correctly in the UI:
  ...
  - Height — CRITICAL for correct rendering in our chat iframe:
    ...
    - Plotly header overlap — CRITICAL: When a chart combines a horizontal legend positioned above the axes (`legend.y > 1`), range-selector buttons (`rangeselector`), and subplot title annotations, all three compete for the same top region and will overlap unless you reserve enough space. Rules: (1) set `fig.update_layout(margin=dict(t=150))` (or more) whenever any two of those elements coexist; (2) place a horizontal legend at `y=1.15` or higher (not `y=1.0`–`1.05`) so it clears the range-selector row; (3) for annotation-based subplot titles use `y` values ≥ 0.05 above the legend's `y` value; (4) if a chart also has range-sliders at the bottom, add sufficient bottom margin (`margin.b ≥ 80`) to avoid clipping the slider. Never rely on Plotly's default top margin of 80 px when the header contains more than a single title.
...
- CRITICAL: NEVER reference a `sandbox:/mnt/data/<filename>` link unless you have already executed code in this response that saved that exact file. Referencing a file that was not created by executed code will result in a broken link.
  - Self-check before writing any sandbox link: look back at the code blocks you executed in this response. Is there a block that explicitly saves this exact filename to `/mnt/data/`? If not, execute the code first — then reference the file.
...
DEFAULT_TOOL_DESCRIPTION_FOR_SYSTEM_PROMPT_FENCE = (
    _TOOL_DESCRIPTION_FOR_SYSTEM_PROMPT_FENCE_BASE + _TOOL_DESCRIPTION_FILES_SUFFIX
).strip()
