...
## User Uploaded Files
The following files have been uploaded by the user and are available in the environment:
{% for file in user_uploaded_files %}
- {{ file }}
{% endfor %}
{% endif %}

{%- if kb_uploaded_files and kb_uploaded_files | length > 0 %}

## Knowledge Base Files
These files are made available by the system but were NOT uploaded by the user:
{% for file in kb_uploaded_files %}
- {{ file }}
{% endfor %}
{% endif %}

{%- if code_interpreter_artifacts and code_interpreter_artifacts | length > 0 %}


## Code Interpreter Artifacts
The following files have been generated by you using the python tool in previous turns:
{% for file in code_interpreter_artifacts %}
- {{ file }}
{% endfor %}
{% endif %}
""".strip()
...
{%- if user_uploaded_files and user_uploaded_files | length > 0 %}
