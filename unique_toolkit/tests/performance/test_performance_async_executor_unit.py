...
from unique_toolkit.performance.async_executor import AsyncExecutor
...
class TestAsyncExecutor(unittest.TestCase):
    ...
    async def test_run_async_tasks_in_threads(self, mock_thread_pool):
        ...
        tasks = [self.async_task(i) for i in range(3)]
        results = await self.executor.run_async_tasks_in_threads(tasks, max_threads=2, max_tasks=2)
    ...
    async def test_run_async_tasks_in_threads_with_exception(self, mock_new_event_loop):
        ...
        with self.assertRaises(Exception) as context:
            await self.executor.run_async_tasks_in_threads(tasks, max_threads=3, max_tasks=1)
