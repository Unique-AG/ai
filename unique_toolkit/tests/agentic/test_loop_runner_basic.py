...
class TestQwenModelHandling:
    ...
    async def test_forced_tools__appends_instruction__for_qwen_model(
        self,
        mock_stream: AsyncMock,
        runner: BasicLoopIterationRunner,
        mock_streaming_handler: MagicMock,
        ...
        """
        Purpose: Verify Qwen tool instruction is appended for Qwen models.
        Why this matters: Qwen models ignore tool_choice and need explicit instruction.
        """
        # Arrange
        mock_stream.return_value = create_stream_response()
        messages = LanguageModelMessages(
            root=[LanguageModelUserMessage(content="Search for documents")]
        )
        tool_choices: list[ChatCompletionNamedToolChoiceParam] = [
            {"type": "function", "function": {"name": "SearchTool"}}
        ]
        ...
        # Assert
        call_kwargs = mock_stream.call_args.kwargs
        loop_kwargs = call_kwargs.get("loop_runner_kwargs", {})
        modified_messages = loop_kwargs.get("messages")
        # The last user message should have the instruction appended
        assert modified_messages is not None
        user_msg = modified_messages.root[-1]
        assert "Tool Call Instruction" in user_msg.content
    ...
    async def test_forced_tools__does_not_modify_messages__for_non_qwen_model(
        self,
        mock_stream: AsyncMock,
        runner: BasicLoopIterationRunner,
        mock_streaming_handler: MagicMock,
        ...
        """
        Purpose: Verify messages are not modified for non-Qwen models.
        Why this matters: GPT models should use standard tool_choice parameter.
        """
        # Arrange
        original_content = "Search for documents"
        mock_stream.return_value = create_stream_response()
        messages = LanguageModelMessages(
            root=[LanguageModelUserMessage(content=original_content)]
        )
        tool_choices: list[ChatCompletionNamedToolChoiceParam] = [
            {"type": "function", "function": {"name": "SearchTool"}}
        ]
