...
# ---------------------------------------------------------------------------
# UN-19267 — large-file download regression tests
# ---------------------------------------------------------------------------


@pytest.mark.ai
def test_config__initial_download_delay_and_connect_timeout__have_correct_defaults() -> (
    None
):
    """
    Purpose: Verify the new UN-19267 config fields have correct default values.
    Why this matters: initial_download_delay_seconds is 0 by default (no unnecessary lag);
    download_connect_timeout replaces the old hard-coded 5 s with 30 s.
    Setup summary: Instantiate config with no args; assert new field defaults.
    """
    config = DisplayCodeInterpreterFilesPostProcessorConfig()

    assert config.initial_download_delay_seconds == 0.0
    assert config.download_connect_timeout == 30.0


@pytest.mark.ai
@pytest.mark.asyncio
async def test_download_file_bytes__applies_initial_delay__before_first_attempt() -> (
    None
):
    """
    Purpose: Verify initial_download_delay_seconds causes asyncio.sleep before download.
    Why this matters: UN-19267 — large files are not immediately available in OpenAI
    container storage; a pre-download pause lets the blob be committed before we try.
    Setup summary: Set initial_download_delay_seconds=0.05; mock sleep; assert called.
    """
    import asyncio
    ...
    proc = _make_display_files_postprocessor()
    proc._config = DisplayCodeInterpreterFilesPostProcessorConfig(
        initial_download_delay_seconds=0.05,
        download_retry_base_delay=0,
    )
    annotation = _make_annotation("long_script.py")
    mock_stream = _MockStreamResponse(b"x" * 1024, content_length=1024)
    proc._client.containers.files.content.with_streaming_response.retrieve = MagicMock(
        return_value=mock_stream
    )
    ...
    async def _capture_sleep(delay: float) -> None:
        ...
        # Only actually sleep for very short delays to keep test fast.
        if delay < 1:
            await original_sleep(delay)

    with patch(
        "unique_toolkit.agentic.tools.openai_builtin.code_interpreter.postprocessors.generated_files.asyncio.sleep",
        side_effect=_capture_sleep,
    ):
        result = await proc._download_file_bytes_with_progress(annotation, tracker=None)

    assert result == b"x" * 1024
    # The initial delay must be the first sleep, before any retry delays.
    assert sleep_calls[0] == pytest.approx(0.05, abs=1e-9)


@pytest.mark.ai
@pytest.mark.asyncio
async def test_download_file_bytes__no_initial_delay__when_configured_to_zero() -> None:
    """
    Purpose: Verify initial_download_delay_seconds=0 skips the pre-download sleep.
    Why this matters: Operators can opt out of the delay for latency-sensitive setups.
    Setup summary: Set initial_download_delay_seconds=0; mock sleep; assert not called
    for the initial delay (ticker sleeps are the only ones permitted).
    """
    from unittest.mock import patch

    proc = _make_display_files_postprocessor()
    proc._config = DisplayCodeInterpreterFilesPostProcessorConfig(
        initial_download_delay_seconds=0,
        download_retry_base_delay=0,
    )
    annotation = _make_annotation("output.txt")
    mock_stream = _MockStreamResponse(b"tiny", content_length=4)
    proc._client.containers.files.content.with_streaming_response.retrieve = MagicMock(
        return_value=mock_stream
    )
    ...
    # No sleep call at all (no initial delay, no retries, no ticker without tracker).
    assert sleep_calls == []


@pytest.mark.ai
@pytest.mark.asyncio
async def test_download_file_bytes__large_payload__succeeds_end_to_end() -> None:
    """
    Purpose: Verify that a large file (~750 KB) is fully downloaded without truncation.
    Why this matters: UN-19267 — large Python scripts generated by code interpreter were
    failing to download. Validates the streaming/chunk accumulation path for big payloads.
    Setup summary: Stream 750 KB of data in 8 KB chunks; assert all bytes returned.
    """

    LARGE_SIZE = 750 * 1024  # 750 KB — matches the failing real-world case
    large_data = b"A" * LARGE_SIZE

    proc = _make_display_files_postprocessor()
    proc._config = DisplayCodeInterpreterFilesPostProcessorConfig(
        initial_download_delay_seconds=0,
        download_retry_base_delay=0,
        progress_update_interval=100.0,  # suppress tracker publish calls in test
    )
    annotation = _make_annotation("long_script.py")
    mock_stream = _MockStreamResponse(large_data, content_length=LARGE_SIZE)
    proc._client.containers.files.content.with_streaming_response.retrieve = MagicMock(
        return_value=mock_stream
    )
...
@pytest.mark.ai
@pytest.mark.asyncio
async def test_download_and_upload__large_payload__returns_content_info() -> None:
    """
    Purpose: Verify the full download-upload pipeline succeeds for a large file (~750 KB).
    Why this matters: UN-19267 — the pipeline was silently returning None for large files,
    causing the 'File download failed' message to be shown to users.
    Setup summary: Stream 750 KB; upload succeeds; assert content_info returned.
    """
    import asyncio

    LARGE_SIZE = 750 * 1024
    large_data = b"B" * LARGE_SIZE
    ...
    mock_upload_result = MagicMock()
    mock_upload_result.id = "cid_large_file"
    proc._chat_service.upload_to_chat_from_bytes_async = AsyncMock(
        return_value=mock_upload_result
    )
    ...
    assert result is not None
    assert result.filename == "long_script.py"
    assert result.content_id == "cid_large_file"
    proc._chat_service.upload_to_chat_from_bytes_async.assert_awaited_once()
    _, kwargs = proc._chat_service.upload_to_chat_from_bytes_async.call_args
    assert kwargs["content"] == large_data


@pytest.mark.ai
@pytest.mark.asyncio
async def test_stream_download__raises_runtime_error__on_non_200_status() -> None:
    """
    Purpose: Verify _stream_download_bytes raises RuntimeError on non-200 HTTP status.
    Why this matters: Previously the status code was logged but iteration would proceed
    silently over an error body. Now we raise early so the retry loop can handle it and
    the exact status is visible in logs.
    Setup summary: Mock a 404 response; assert RuntimeError raised with status in message.
    """
    import asyncio
    ...
    # failsafe_async catches the RuntimeError and returns None.
    assert result is None


# ---------------------------------------------------------------------------
# _kb_safe_mime — UN-19267 MIME type coercion
# ---------------------------------------------------------------------------
