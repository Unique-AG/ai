...
def ensure_sub_agent_tool_registered():
    """
    Ensure SubAgentTool is registered in ToolFactory before tests run.

    This is necessary because other test modules may clear the ToolFactory
    state, which removes the registration that happened at module import time.
    """
    if "SubAgentTool" not in ToolFactory.tool_config_map:
