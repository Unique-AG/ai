...
class TestTodoMultiStepWorkflow:
    ...
    @pytest.mark.ai
    @pytest.mark.asyncio
    async def test_system_reminder__reflects_state_after_each_write(
        self, mock_event: ChatEvent, shared_memory: dict
    ) -> None:
        """
        Purpose: Verify the system-reminder changes to reflect updated state.
        Why: The injection must be current for the model to make correct decisions.
        Setup: Two writes, check reminder after each.
        """
        tool = _build_tool(mock_event, shared_memory)
