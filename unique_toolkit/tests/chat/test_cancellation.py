...
def _make_watcher(**overrides) -> CancellationWatcher:
    defaults = dict(
        user_id="u1",
        company_id="c1",
        chat_id="chat1",
    ...
    return CancellationWatcher(**defaults)
...
class TestRunWithCancellation:
    ...
    async def test_returns_cancel_result__when_cancelled(self):
        ...
        check_count = 0
        original_check = w.check_cancellation_async
        ...
        async def mock_check():
            ...
            if check_count >= 1:
                ...
                await w._bus.publish_and_wait(event)
                return True
            return False
