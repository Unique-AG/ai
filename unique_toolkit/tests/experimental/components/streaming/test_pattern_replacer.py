...
def _apply_batch_normalization(text: str) -> str:
    normalized = text
...
def test_AI_streaming_pattern_replacer__bare_source_aliases__normalize_consistently(
    source_token: str, expected: str
) -> None:
    """
    Purpose: Verify all bare source aliases use one consistent normalization rule.
    Why this matters: Overlapping source regexes can make uppercase and underscore variants diverge.
    Setup summary: Feed a bare source token through streaming normalization; expect a superscript.
    """
    replacer = StreamingPatternReplacer(
        NORMALIZATION_PATTERNS, max_match_length=NORMALIZATION_MAX_MATCH_LENGTH
    )
    combined = replacer.process(source_token) + replacer.flush()
...
def test_AI_batch_normalization_patterns__bare_source_aliases__normalize_consistently(
    source_token: str, expected: str
) -> None:
    """
    Purpose: Verify batch normalization accepts the same bare source aliases as streaming.
    Why this matters: Batch and streaming citation parsing should not drift for case or separators.
    Setup summary: Apply batch normalization rules to one source token; expect bracket notation.
    """
    assert _apply_batch_normalization(source_token) == expected
...
def test_streaming_pattern_replacer__max_match_length_zero__releases_full_delta_each_call():
    ...
    Setup summary: With max_match_length=0 the implementation applies replacements immediately;
...
def test_streaming_pattern_replacer__callable_replacement__used_for_multi_source():
    """
    Purpose: Ensure callable replacements run in the streaming event routing.
