...
def test_metrics(fresh_registry: CollectorRegistry):
    """Provide a pair of (duration Histogram, errors Counter) for track() tests."""
    duration = Histogram(
...
def test_track__records_duration__on_success(test_metrics) -> None:
    """
    Purpose: Verify track() observes a duration measurement after a successful block.
    Why this matters: Without duration recording, latency dashboards show no data.
    Setup summary: Run a trivial block inside track(), assert histogram sample count increases.
    """
    duration, _ = test_metrics

    before = duration.labels(op="test")._sum.get()

    with track(duration, op="test"):
        pass
...
def test_track__records_error__on_exception(test_metrics) -> None:
    """
    Purpose: Verify track() increments the error counter when the block raises.
    Why this matters: Without error recording, failures are invisible in Prometheus dashboards.
    Setup summary: Raise ValueError inside track(), assert errors counter incremented with correct labels.
    """
    duration, errors = test_metrics

    with pytest.raises(ValueError):
        with track(duration, errors, op="test"):
...
def test_track__still_records_duration__on_exception(test_metrics) -> None:
    """
    Purpose: Verify track() records duration even when the block raises an exception.
    Why this matters: Latency data for failed requests is valuable for debugging.
    Setup summary: Raise inside track(), assert duration histogram still observed.
    """
    duration, errors = test_metrics
...
def test_track__no_errors_counter__does_not_raise__on_exception(
    ...
    """
    Purpose: Verify track() works without an errors counter (errors=None).
    Why this matters: Some call sites only care about duration, not errors — errors=None must be safe.
    Setup summary: Pass errors=None to track(), raise inside block, assert original exception propagates.
    """
    duration, _ = test_metrics
