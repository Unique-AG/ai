"""Tests for ``ResponsesTextDeltaEventHandler`` — accumulation and flushing rules
...
@pytest.mark.ai
@pytest.mark.asyncio
async def test_AI_responses_text_delta_event_handler__on_stream_end__does_not_publish_trailing_flush():
    """
    ...
    event_handler = ResponsesTextDeltaEventHandler(replacers=[replacer])  # type: ignore[list-item]
    received: list[TextFlushed] = []
    event_handler.text_bus.subscribe(received.append)

    await event_handler.on_text_delta(_delta("hi"))
