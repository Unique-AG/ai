...
@dataclass
class _FakeChatTextEventHandler:
    """Fake text event handler: records chunks and publishes configurable flushes.

    The new protocol replaces the bool return value with an event-handler-owned
...
@dataclass
class _FakeChatToolEventHandler:
...
@pytest.mark.ai
@pytest.mark.asyncio
async def test_AI_chat_completion_stream_event_router__on_event__forwards_to_both_event_handlers():
    """
    Purpose: Verify each chunk is passed to text and tool event handlers.
    Why this matters: Chat chunks may carry content and tool deltas together; both paths must run.
    Setup summary: Fake event handlers; await on_event with a chunk; assert both recorded the same event.
    """
    text_h = _FakeChatTextEventHandler()
    tool_h = _FakeChatToolEventHandler()
    router = ChatCompletionStreamEventRouter(
        text_event_handler=text_h, tool_call_event_handler=tool_h
    )
...
async def test_AI_chat_completion_stream_event_router__on_event__propagates_flush_via_bus():
    ...
    Setup summary: Fake text event handler configured to flush; subscribe to the
      ...
      is received.
    """
    text_h = _FakeChatTextEventHandler(flush=True)
    router = ChatCompletionStreamEventRouter(text_event_handler=text_h)
...
@pytest.mark.ai
@pytest.mark.asyncio
async def test_AI_chat_completion_stream_event_router__on_stream_end__finalizes_all_event_handlers():
    """
    Purpose: Ensure ``on_stream_end`` calls every registered event handler.
    Why this matters: Event handlers flush replacer buffers and release resources.
...
async def test_AI_chat_completion_stream_event_router__on_stream_end__publishes_residual_flush():
    ...
    """
    text_h = _FakeChatTextEventHandler(end_flush=True)
    router = ChatCompletionStreamEventRouter(text_event_handler=text_h)
    received: list[TextFlushed] = []
    router.text_bus.subscribe(received.append)
    await router.on_stream_end()
...
def test_AI_chat_completion_stream_event_router__build_result__includes_tool_calls():
    ...
    Setup summary: Fake event handlers with tool_calls; build_result; assert message text and tools.
    """
    text_h = _FakeChatTextEventHandler()
    tool_fn = LanguageModelFunction(name="t", arguments={"a": 1})
    tool_h = _FakeChatToolEventHandler(tools=[tool_fn])
    router = ChatCompletionStreamEventRouter(
        text_event_handler=text_h, tool_call_event_handler=tool_h
    )
...
def test_AI_chat_completion_stream_event_router__get_text__delegates_to_text_event_handler():
    ...
    """
    text_h = _FakeChatTextEventHandler(
        state=TextState(full_text="a", original_text="b")
    )
    router = ChatCompletionStreamEventRouter(text_event_handler=text_h)
    s = router.get_text()
...
async def test_AI_responses_stream_event_router__on_event__routes_text_delta_and_publishes_flush():
    ...
    router = ResponsesStreamEventRouter(
        text_event_handler=text_h, completed_event_handler=done_h
    )
    received: list[TextFlushed] = []
    router.text_bus.subscribe(received.append)
    ev = _text_delta("abc")
...
async def test_AI_responses_stream_event_router__on_event__non_text_events_do_not_publish_flush():
    ...
    tools = ResponsesToolCallEventHandler()
    router = ResponsesStreamEventRouter(
        text_event_handler=text_h, tool_call_event_handler=tools
    )
    received: list[TextFlushed] = []
    router.text_bus.subscribe(received.append)
...
async def test_AI_responses_stream_event_router__tool_flow__records_function_call():
    """
    Purpose: Output-item + arguments-done events should produce a tool call on the event handler.
    Why this matters: Downstream execution needs parsed name and arguments from the stream.
    Setup summary: Real ResponsesToolCallEventHandler; simulate added item then arguments JSON.
    """
    text_h = _FakeResponsesText()
    tools = ResponsesToolCallEventHandler()
    router = ResponsesStreamEventRouter(
...
@pytest.mark.ai
@pytest.mark.asyncio
async def test_AI_responses_stream_event_router__on_stream_end__invokes_all_event_handlers():
    """
    Purpose: on_stream_end must finalize every registered event handler.
    Why this matters: Event handlers flush buffers and release resources.
...
async def test_AI_responses_stream_event_router__on_stream_end__publishes_residual_flush():
    ...
    router = ResponsesStreamEventRouter(text_event_handler=text_h)
    received: list[TextFlushed] = []
    router.text_bus.subscribe(received.append)
    await router.on_stream_end()
    assert len(received) == 1
...
@pytest.mark.ai
@pytest.mark.asyncio
async def test_AI_responses_completed_event_handler__on_completed__extracts_usage_and_output():
    """
    ...
    await event_handler.on_completed(event)  # type: ignore[arg-type]
    u = event_handler.get_usage()
    assert u is not None
    assert u.prompt_tokens == 3
    assert u.completion_tokens == 5
    assert u.total_tokens == 8
    assert event_handler.get_output() == [out_item]
...
def test_AI_responses_stream_event_router__build_result__includes_usage_and_original_text():
    """
    Purpose: build_result should surface text, usage, and output from event handlers.
