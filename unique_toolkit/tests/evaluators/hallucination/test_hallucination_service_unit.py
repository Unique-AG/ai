...
class TestHallucinationEvaluator(TestCase):
    ...
    @pytest.mark.asyncio
    @patch("unique_toolkit.evaluators.hallucination.service.check_hallucination_async")
    async def test_analyze_metric_enabled(self, mock_check_hallucination_async):
    ...
    @pytest.mark.asyncio
    @patch("unique_toolkit.evaluators.hallucination.service.check_hallucination_async")
    async def test_analyze_with_default_config(self, mock_check_hallucination_async):
        expected_result = MagicMock(spec=EvaluationMetricResult)
        mock_check_hallucination_async.return_value = expected_result
    ...
    @pytest.mark.asyncio
    @patch("unique_toolkit.evaluators.hallucination.service.check_hallucination_async")
    async def test_analyze_exception_handling(self, mock_check_hallucination_async):
        self.mock_config.enabled = True
