"""Unit tests for :class:`~unique_toolkit.content.tree.service.ContentTree`."""
...
# All patches target the name as bound in ``content.tree.service`` (where the
# service module imports it from ``functions``), not the original definition
# module. That's the usual ``mock.patch`` rule: patch where it's looked up.
_PATCH_TARGET = "unique_toolkit.content.tree.service.resolve_visible_file_paths_core"
