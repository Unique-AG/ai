...
The :class:`~unique_toolkit.content.tree.service.ContentTree` builds a **filesystem-style tree** of knowledge-base **content the acting user can see**. It is **not** the full folder hierarchy from :class:`~unique_toolkit.content.folder.service.ContentFolder`—only folders that appear on visible content paths (via ``folderIdPath`` metadata) show up.
...
!!! note "Environment"
    ...
    Use the same SDK / :class:`~unique_toolkit.app.unique_settings.UniqueSettings` setup as other toolkit examples (`UNIQUE_API_KEY`, `UNIQUE_APP_ID`, user and company context). The sample script uses :meth:`~unique_toolkit.content.tree.service.ContentTree.from_settings`.
...
Use :meth:`~unique_toolkit.content.tree.service.ContentTree.render_visible_tree_async`. Pass ``max_depth=None`` for an unlimited tree, or an integer for **``tree -L``-style** truncation: depth ``1`` shows only top-level folders and files; deeper levels are summarized as a single “…” line with counts when cut off.
...
If you want the raw list of visible files (not a tree), use :meth:`~unique_toolkit.content.tree.service.ContentTree.list_visible_files_async`. For predicates the server-side ``metadata_filter`` can't express — arbitrary Python over :class:`~unique_toolkit.content.schemas.ContentInfo` — use :meth:`~unique_toolkit.content.tree.service.ContentTree.filter_visible_files_async`:
...
:meth:`~unique_toolkit.content.tree.service.ContentTree.search_visible_files_fuzzy_async` matches a query against the file basename, the joined folder path, or both. Scoring uses stdlib :class:`difflib.SequenceMatcher`; matching is **case-insensitive by default**. Results come back as :class:`~unique_toolkit.content.tree.schemas.FuzzyMatch` records sorted by descending score:
...
``metadata_filter`` is forwarded to content listing (same idea as smart rules / filters in the [Knowledge Base service](kb_service.md) and [Smart Rules](smart_rules.md)). Example shape:
...
If you already use :class:`~unique_toolkit.services.factory.UniqueServiceFactory`:

```{.python #kb-tree-factory-snippet}
from unique_toolkit.app.unique_settings import UniqueSettings
from unique_toolkit.services.factory import UniqueServiceFactory

settings = UniqueSettings.from_env_auto_with_sdk_init()
tree_svc = UniqueServiceFactory(settings=settings).content_tree()
```

## Related

- [Content folder service](content-folder.md) — create/delete **folders** (scopes) and ACL.
- [Knowledge Base service examples](kb_service.md) — upload, search, and filter **content**.
