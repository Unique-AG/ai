...
The **expandable** script below is generated from the same blocks: run ``bash generate_examples.sh`` in the ``unique_toolkit`` package root whenever you change the Markdown (naked ``entangled tangle``, copy to ``docs/examples_from_docs/``, ``isort``/``ruff``, then ``# %%``).
...
After mirroring, we render the **visible content tree** with :class:`~unique_toolkit.content.tree.service.ContentTree` so you can **verify the result** directly on the command line. The tree shows folders that appear on visible content paths (see the [content tree](content-tree.md) page); because :meth:`~unique_toolkit.content.tree.service.ContentTree.render_visible_tree_async` is asynchronous, the entry point is wrapped in ``asyncio.run``.
