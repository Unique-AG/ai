...
#
# This demonstrates the Python-side streaming event routing for the OpenAI Responses API.
...
#
# The event handler streams tokens directly through the OpenAI proxy, normalises
...
from unique_toolkit.integrations.openai.streaming.event_routing import (
    ResponsesCompleteWithReferences,
    ResponsesStreamEventRouter,
)
from unique_toolkit.integrations.openai.streaming.event_routing.responses import (
...
for event in get_event_generator(unique_settings=settings, event_type=ChatEvent):
    ...
    # --- Build event handlers and stream event router -----------------------------
    ...
    text_event_handler = ResponsesTextDeltaEventHandler(
        replacers=replacers,
    )
    ...
    router = ResponsesStreamEventRouter(
        text_event_handler=text_event_handler,
        tool_call_event_handler=tool_call_event_handler,
        completed_event_handler=completed_event_handler,
    ...
    event_handler = ResponsesCompleteWithReferences(
        event_settings,
        router=router,
    )
    ...
    result = event_handler.complete_with_references(
