...
from unique_toolkit.integrations.openai.streaming.event_routing.chat_completions import (
...
async def main():
    ...
    for event in get_event_generator(unique_settings=settings, event_type=ChatEvent):
        ...
        # Build event handlers and event routing
        # --------------------------------
        ...
        text_event_handler = ChatCompletionTextEventHandler(
            replacers=replacers,
        ...
        streaming_event_handler = ChatCompletionsCompleteWithReferences(
            settings=settings,
            router=router,
        )

        result = await streaming_event_handler.complete_with_references_async(
            messages=messages,
