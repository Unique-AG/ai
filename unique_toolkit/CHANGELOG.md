...
## [1.62.3] - 2026-03-24
- Add `contentId` to search result source dicts in tool call responses (`transform_chunks_to_string` and `_create_reduced_standard_sources_message`) so the LLM can associate each source with its content object
...
>>>>>>> 2681713b (changed from contentId to content_id)
