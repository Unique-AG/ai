...
## [1.51.0] - 2026-03-10
- Make `ToolBuildConfig` generic over the configuration type (`ToolBuildConfig[T]` where `T` is bound to `BaseToolConfig`, default `BaseToolConfig`). Enables downstream consumers to parameterize the configuration type without invariant-override type errors. Fully backward compatible — unparameterized usage behaves identically.
