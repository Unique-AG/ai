...
_DEFAULT_EXECUTION_REMINDER = (
    ...
    "Always include a todo_write call alongside your work tool calls "
    "in the same response — mark items in_progress before executing "
    "and completed after. Do not use a separate turn just for status "
    "updates. "
    "Do NOT write a final text response while items are still pending "
    "or in_progress — keep executing until every item reaches a "
    "terminal state (completed/cancelled), then call todo_write one "
    "final time before responding."
)
