...
## [1.14.1] - 2026-03-14
- Add `ClaudeAgentConfig | None` field to `ExperimentalConfig` for explicit opt-in Claude Agent SDK routing (Decision B6 — never auto-enabled by model name)
- Add `_build_claude_agent()` factory to `unique_ai_builder.py`; `build_unique_ai()` returns `UniqueAI | ClaudeAgentRunner` and checks Claude route before responses_api route
