...
`ls` returns **one page of folders and one page of files** per call (currently 50
of each). When a folder holds more, the output says so explicitly and gives you
the command for the next page — always take the range and the next command from
that notice rather than assuming a page size:
...
A summary line reading `50 of 212 file(s)` means 50 were listed out of 212 that
exist. A plain `12 file(s)` (no "of") means the listing is complete. `--skip` past
the end reports the real totals instead of looking like an empty folder.

To glance at just the first entries, pipe the output — only what you pipe enters
your context:
...
There is no `--take`: the page size is the server's, because choosing one would
require already knowing how big the folder is.
