...
Unified web egress proxy for search engines and crawlers. One Python package, **two layers** that must not be confused:

| Layer | Module | Process | HTTP role |
|-------|--------|---------|-----------|
| **Application** | `unique_search_proxy.web` | Proxy pod (FastAPI + uvicorn) | **Server** — `/v1/search`, `/v1/crawl`, configuration, metrics |
| **SDK** | `unique_search_proxy.sdk` | Tool caller pod (e.g. `unique_web_search`) | **Client** — httpx to the proxy base URL only |
...
- **Application** owns provider registry, secrets, Prometheus, and `web.core.client.HttpClientPool` (outbound crawl/search).
- **SDK** is a hand-maintained wrapper around the [OpenAPI](http://localhost:2349/docs) contract. It reuses **Pydantic API models** from `web.api.v1.schema` for typing, but does **not** call engine/crawler services in-process.

Shared types (engine/crawler config models) live under `web.core` and are imported by both layers; that is intentional so config shapes stay aligned.
...
## Python SDK (`unique_search_proxy.sdk`)

The SDK is **generated from the FastAPI OpenAPI spec** using [openapi-python-client](https://github.com/openapi-generators/openapi-python-client), then wrapped for ergonomics and proxy error types.

| Path | Role |
|------|------|
| `unique_search_proxy/sdk/_generated/` | Regenerated httpx client + attrs models (do not edit by hand) |
| `unique_search_proxy/sdk/client.py` | `UniqueSearchProxyClient` facade + `ProxyError` mapping |
| `openapi.json` | Exported spec (input to codegen) |
...
# or: uv run poe generate-sdk   (requires poethepoet in the environment)
```
