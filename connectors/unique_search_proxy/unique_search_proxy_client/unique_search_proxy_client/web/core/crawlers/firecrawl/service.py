...
class FirecrawlCrawlerService(BaseCrawler[FirecrawlCrawlRequest]):
    """Firecrawl batch scrape crawler (httpx POST + poll)."""
    ...
    async def crawl(self, request: FirecrawlCrawlRequest) -> list[CrawlUrlResult]:  # type: ignore[override]
        ...
        if start_response.status_code >= 400:
            message = _upstream_error_message(start_response)
            return [_error_result(url, message) for url in urls]
        ...
        return _map_firecrawl_results(urls, final_payload)
...
def _upstream_error_message(response: httpx.Response) -> str:
    try:
        ...
        return f"Firecrawl batch scrape failed with HTTP {response.status_code}"
