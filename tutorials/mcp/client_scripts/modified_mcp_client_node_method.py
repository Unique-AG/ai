...
Note: this build pins ``token_endpoint_auth_method="none"`` during Dynamic
Client Registration so the registration payload mirrors what the node-chat
MCP client sends, which is helpful when reproducing parity issues.
"""
...
# Mirrors node-chat's `clientMetadata.token_endpoint_auth_method = 'none'` in
# next/services/node-chat/src/mcp/mcp.service.ts so the DCR payload is
# byte-identical between the two clients.
NODE_CHAT_PARITY_CLIENT_METADATA: dict[str, object] = {
...
class TracingOAuth(FastMCPOAuth):  # type: ignore[misc, valid-type]
    ...
    def __init__(self, *, body_preview_chars: int) -> None:
        # Pass `token_endpoint_auth_method="none"` through FastMCP's
        # `additional_client_metadata` hook so the DCR payload sent here matches
        # node-chat's `clientMetadata.token_endpoint_auth_method = 'none'`
        # (mcp.service.ts). FastMCP otherwise omits the field, letting the
        # auth server pick its preferred method.
        super().__init__(additional_client_metadata=NODE_CHAT_PARITY_CLIENT_METADATA)
...
async def connect_with_fastmcp(mcp_url: str, auth: FastMCPAuth, *, body_preview_chars: int) -> None:
    ...
    if auth == "oauth":
        ...
        print(
            "DCR client metadata override: "
            f"{json.dumps(NODE_CHAT_PARITY_CLIENT_METADATA, sort_keys=True)} "
            "(matching node-chat)"
        )
...
async def main() -> int:
    ...
    print_kv("dcr_client_metadata_override", json.dumps(NODE_CHAT_PARITY_CLIENT_METADATA, sort_keys=True))
