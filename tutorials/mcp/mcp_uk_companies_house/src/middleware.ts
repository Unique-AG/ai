...
// Token endpoint
router.post("/token", express.urlencoded({ extended: false }), (req, res, next) => {
  if (!authEnabled()) return next();
  ...
  if (authHeader?.startsWith("Basic ")) {
    ...
    if (i !== -1 && decoded.slice(0, i) === process.env.MCP_CLIENT_ID && decoded.slice(i + 1) === process.env.MCP_CLIENT_SECRET) {
      clientId = decoded.slice(0, i);
    }
  } else if (req.body?.client_id === process.env.MCP_CLIENT_ID && req.body?.client_secret === process.env.MCP_CLIENT_SECRET) {
  ...
  if (grantType === "authorization_code") {
    ...
    res.json({ access_token: accessToken, token_type: "bearer", expires_in: TOKEN_LIFETIME });
    return;
  }
...
const middleware: RequestHandler = (req, res, next) => {
  ...
  if (!authHeader?.startsWith("Bearer ")) {
    const base = getBaseUrl(req);
    res.status(401)
      .setHeader("WWW-Authenticate", `Bearer resource_metadata="${base}/.well-known/oauth-protected-resource"`)
