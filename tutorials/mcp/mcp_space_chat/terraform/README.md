# MCP Space Chat - Azure Terraform Deployment

This Terraform configuration deploys the MCP Space Chat application to Azure with the following resources:
...
|----------|-------------|---------|
| Container Registry | `acrspacechatmcp<suffix>` | Stores Docker images |
| Key Vault | `kv-space-chat-mcp-<suffix>` | Stores application secrets |
| Storage Account | `stspacechatmcp<suffix>` | Stores Caddy certificates (prod/staging) + Terraform state |
| Log Analytics Workspace | `law-space-chat-mcp-<suffix>` | Centralized logging (30 days retention) |
| Container Instance | `aci-space-chat-mcp-<suffix>` | Runs the application containers |
| Managed Identity | `id-space-chat-mcp-<suffix>` | Provides secure access to resources |
...
az acr build --registry $(terraform output -raw acr_name) \
  --image mcp-space-chat:$(git rev-parse --short HEAD)-$(date +%Y%m%d-%H%M%S) \
  --image mcp-space-chat:latest .
...
| `base_name` | Base name for resources with hyphens (e.g., `space-chat-mcp` → `kv-space-chat-mcp-xxx`) |
| `base_name_clean` | Base name without hyphens for Storage/ACR (e.g., `spacechatmcp` → `stspacechatmcpxxx`) |
...
| `unique_frontend_base_url` | UNIQUE_FRONTEND_BASE_URL — Unique web app origin for `/chat/embed` (required) |
...
```bash
export TF_VAR_resource_group_name="rg-mcp-space-chat"
export TF_VAR_location="westeurope"
export TF_VAR_base_name="space-chat-mcp"
export TF_VAR_base_name_clean="spacechatmcp"
export TF_VAR_domain_name="mcp-space-chat.yourdomain.com"
...
curl http://$(terraform output -raw aci_fqdn):8004/health

# Test MCP protocol endpoint
curl -X POST http://$(terraform output -raw aci_fqdn):8004/mcp \
...
./deploy.sh logs mcp-space-chat   # MCP Space Chat application logs
./deploy.sh logs caddy        # Caddy reverse proxy logs

# Follow logs in real-time (live updates)
./deploy.sh logs mcp-space-chat -f
...
./deploy.sh logs mcp-space-chat --analytics
...
When you use the Azure Container Instance FQDN (e.g., `aci-space-chat-mcp-xxx.swedencentral.azurecontainer.io`):
...
1. **Use a domain you own**:
   - Create a subdomain (e.g., `mcp-space-chat.yourdomain.com`)
   - Point DNS to your ACI IP: `terraform output aci_ip_address`
   - Update `terraform.tfvars`: `domain_name = "mcp-space-chat.yourdomain.com"`
...
terraform {
  backend "azurerm" {
    resource_group_name  = "rg-mcp-space-chat"           # Your resource group
    storage_account_name = "stspacechatmcp<xxxxxx>"     # From terraform output
    container_name       = "tfstate"                  # Container created automatically
    key                  = "mcp-space-chat.tfstate"       # State file name
  }
}
```
...
docker build -t ${ACR_SERVER}/mcp-space-chat:${IMAGE_TAG} ..
docker tag ${ACR_SERVER}/mcp-space-chat:${IMAGE_TAG} ${ACR_SERVER}/mcp-space-chat:latest

# Push both tags to ACR
docker push ${ACR_SERVER}/mcp-space-chat:${IMAGE_TAG}
docker push ${ACR_SERVER}/mcp-space-chat:latest
...
2. **Storage Account**: The storage account name includes this suffix (e.g., `stspacechatmcpp33drs`)
