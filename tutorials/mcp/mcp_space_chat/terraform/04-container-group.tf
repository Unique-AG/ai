...
#             identity.tf, container-mcp-space-chat.tf, container-caddy.tf
# 
...
resource "azurerm_container_group" "main" {
  ...
  # -------------------------------------------------------------------------
  # MCP Space Chat Application Container
  # Configuration defined in: 04-container-mcp-space-chat.tf
  # -------------------------------------------------------------------------
  container {
    name   = local.mcp_space_chat_container.name
    image  = local.mcp_space_chat_container.image
    cpu    = local.mcp_space_chat_container.cpu
    memory = local.mcp_space_chat_container.memory

    dynamic "ports" {
      for_each = local.mcp_space_chat_container.ports
      content {
    ...
    environment_variables = local.mcp_space_chat_container.environment_variables

    secure_environment_variables = local.mcp_space_chat_container.secure_environment_variables
  }
