# ============================================================================
# MCP Space Chat Container
# Depends on: foundation.tf, container-registry.tf, key-vault.tf, identity.tf
# 
# This file defines everything the MCP Space Chat container needs:
...
locals {
  mcp_space_chat_container = {
    name   = var.base_name
    image  = "${azurerm_container_registry.main.login_server}/mcp-space-chat:${var.app_image_tag}"
    ...
    environment_variables = {
      # Server settings (unique_mcp.ServerSettings — UNIQUE_MCP_ prefix)
      "UNIQUE_MCP_PUBLIC_BASE_URL" = "https://${var.domain_name}"
      "UNIQUE_MCP_LOCAL_BASE_URL"  = "http://0.0.0.0:8004"
      # Chat embed origin (required — nested iframe target for MCP Apps / MCP-UI)
      "UNIQUE_FRONTEND_BASE_URL" = var.unique_frontend_base_url
    }
