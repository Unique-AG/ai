#!/bin/bash
# ============================================================================
# MCP Space Chat - Azure Deployment Verification Script
# ============================================================================
# This script comprehensively verifies that Terraform has properly deployed
# the MCP Space Chat application to Azure by checking:
# - Terraform state
    ...
    if [ "$CONTAINERS" != "[]" ]; then
        ...
        if [ -z "$MCP_CONTAINER_NAME" ]; then
            ...
            if command -v jq &> /dev/null; then
                ...
                if echo "$CONTAINERS" | grep -q '"name":"space-chat-mcp"'; then
                    MCP_CONTAINER_NAME="space-chat-mcp"
                elif echo "$CONTAINERS" | grep -q '"name":"mcp-space-chat"'; then
                    MCP_CONTAINER_NAME="mcp-space-chat"
                fi
            fi
        fi
        
        if [ -z "$MCP_CONTAINER_NAME" ]; then
            log_warn "Could not determine MCP container name, trying 'mcp-space-chat'"
            MCP_CONTAINER_NAME="mcp-space-chat"
        fi
...
verify_health_endpoints() {
    ...
    MCP_CONTAINER_NAME=$(terraform output -raw base_name 2>/dev/null || echo "space-chat-mcp")
    if [ -z "$MCP_CONTAINER_NAME" ]; then
        MCP_CONTAINER_NAME="space-chat-mcp"
    fi
    ...
    if [ -n "$ACI_FQDN" ]; then
        ...
        # Note: Port 8004 is not exposed externally (only accessible from within container group)
        # Caddy proxies requests from port 80/443 to the container on port 8004 internally
        log_info "Note: Direct container access on port 8004 is not exposed externally (by design)"
...
verify_container_registry() {
    ...
    # Check if mcp-space-chat image exists
    if az acr repository show-tags --name "$ACR_NAME" --repository mcp-space-chat &> /dev/null; then
        TAGS=$(az acr repository show-tags --name "$ACR_NAME" --repository mcp-space-chat --output tsv 2>/dev/null | wc -l | tr -d ' ')
        if [ "$TAGS" -gt 0 ]; then
            log_success "mcp-space-chat image exists in registry ($TAGS tag(s))"
        else
            log_warn "mcp-space-chat repository exists but has no tags"
        fi
    else
        log_warn "mcp-space-chat image not found in registry (may need to build/push)"
    fi
...
main() {
    echo ""
    echo "=========================================="
    echo "MCP Space Chat - Azure Deployment Verification"
    echo "=========================================="
    echo ""
