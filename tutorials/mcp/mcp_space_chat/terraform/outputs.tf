...
output "base_name" {
  description = "The base name used for the MCP Space Chat container"
  value       = var.base_name
}
...
output "test_endpoint_command" {
  ...
  value       = <<-EOF
    ...
    # Or test via direct FQDN (Caddy proxies to container on port 8004 internally)
...
output "docker_build_and_push_command" {
  ...
  value       = <<-EOF
    ...
    docker build -t ${azurerm_container_registry.main.login_server}/mcp-space-chat:latest .

    # Push to ACR
    docker push ${azurerm_container_registry.main.login_server}/mcp-space-chat:latest

    # Or use ACR Tasks to build directly in Azure:
    az acr build --registry ${azurerm_container_registry.main.name} --image mcp-space-chat:latest .
  EOF
}
...
output "view_logs_mcp_space_chat" {
  description = "Command to view MCP Space Chat container logs"
...
output "follow_logs_mcp_space_chat" {
  description = "Command to follow MCP Space Chat container logs (live)"
