#!/usr/bin/env python3
"""Verbose OAuth MCP client for debugging the Space Chat server.
...
auth succeeds it lists tools and optionally lists tools / calls list_spaces.

Usage (from this directory)::

    uv run python debug_auth_client.py
    uv run python debug_auth_client.py --url https://unique-space-chat-mcp.azurewebsites.net/mcp
    uv run python debug_auth_client.py --url http://localhost:8003/mcp --query "what is unique?"
    uv run python debug_auth_client.py --no-tools   # auth + list_tools only
...
DEFAULT_MCP_URL = "https://unique-space-chat-mcp.azurewebsites.net/mcp"
DEFAULT_QUERY = ""  # optional name filter for list_spaces
...
def exchange(response: httpx.Response, *, body_limit: int, stream: bool = False) -> None:
...
class TracingOAuth(OAuth):
    ...
    def __init__(self, mcp_url: str, *, body_limit: int, callback_port: int) -> None:
        super().__init__(
            mcp_url=mcp_url,
            client_name="mcp-space-chat-debug-auth-client",
                ...
                while True:
                    ...
                    if _is_auth_exchange(response):
                        ...
                        if "text/event-stream" in content_type:
                            exchange(
                                response, body_limit=self._body_limit, stream=True
                            )
                        else:
...
async def probe_unauthenticated(mcp_url: str, *, body_limit: int, timeout: float) -> None:
        ...
        resp = await client.post(
            ...
            json={
                ...
                "params": {
                    ...
                    "capabilities": {},
                    "clientInfo": {
                        "name": "mcp-space-chat-debug-auth-client",
                        "version": "0.1.0",
                    },
                },
            },
        ...
        for url in urls:
            ...
            if "json" in resp.headers.get("content-type", ""):
                ...
                if isinstance(payload, dict) and "authorization_endpoint" in payload:
                    kv("★ authorization_endpoint", payload.get("authorization_endpoint"))
    ...
    *,
    ...
    with trace_all_httpx(body_limit):
        async with Client(mcp_url, auth=oauth) as client:
            ...
            if query is None:
                print("\n(--no-tools: skipping tool call)", flush=True)
                return

            step(f"5. Call list_spaces(name={query!r})")
            ...
            for i, item in enumerate(content, 1):
                ...
                print(f"\n--- result [{i}] ---", flush=True)
                print(text[:2_000] if isinstance(text, str) else str(text)[:2_000], flush=True)
...
async def amain(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        help="Optional name filter for list_spaces (default: list all).",
    )
    parser.add_argument(
        "--no-tools",
        action="store_true",
        help="Stop after list_tools (do not call list_spaces).",
    )
    parser.add_argument(
        "--callback-port",
        type=int,
    ...
    if not mcp_url.endswith("/mcp"):
        ...
        if mcp_url.endswith(".net") or mcp_url.endswith(".app") or mcp_url.count("/") <= 2:
            mcp_url = mcp_url + "/mcp"

    step("0. Configuration")
    kv("mcp_url", mcp_url)
    kv("callback_port", args.callback_port)
    kv("query", None if args.no_tools else args.query)
    ...
    await connect_and_exercise(
        mcp_url,
        query=None if args.no_tools else args.query,
