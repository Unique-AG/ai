...
class PMPositionsTool(Tool[PMPositionsToolConfig]):
    ...
    async def run(self, tool_call: LanguageModelFunction) -> ToolCallResponse:
        ...
        tool_response = ToolCallResponse(
            ...
            content=str(data),
            debug_info={
                "sql": f"SELECT * FROM (SELECT * FROM {TABLE_NAME} WHERE email = $1) AS tmp {where_clause}",
                "params": ["<email>"],
            },
        )
    ...
    def select_data(self, conn, where_clause, email):
        # SECURITY NOTE: where_clause is generated by an LLM and cannot be fully
        # parameterized since it is a dynamic SQL fragment. The mitigations here
        # (keyword blocklist + read-only transaction) reduce risk but do NOT
