...
datasets/<dataset>/
  ...
  fastmcp/     # Excel, SQLite, generated Pydantic models, import_plan.py, app.py
...
3. Decide the SQLite storage mode. Prefer TypeSpec-aligned curated storage: split wide workbooks into domain tables (or domain-prefixed columns when a single table is enough). Write an Excel import plan that translates workbook sheets/columns into that schema. Expect the server to still map storage onto the nested contract — see `docs/architecture.md#domain-to-storage-mapping`.
...
7. Implement `datasets/<dataset>/fastmcp/app.py` (and supporting modules) by passing dataset-local paths into `AppSettings`:
