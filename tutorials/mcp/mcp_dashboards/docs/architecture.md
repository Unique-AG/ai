...
`account_review` uses curated storage split by TypeSpec groups:

- `clients` — identity columns only (`name`, `reference`, `crd_number`, `type`,
  `segment`).
- 1:1 satellites keyed by `client_id`: `contacts`, `portfolios`, `compliance`,
  `review_schedules`, `suitability`, `case_actions`.
- `figure_metrics` — one row per dashboard metric (`group_name` + `position`),
  replacing the workbook's repeated `fig{1..3}_*` / `hold{1..3}_*` groups.

Reads JOIN the 1:1 tables and project flat aliases (`identity_name`,
`case_action_status`, …) for the domain mapper; writes route each field to its
owning table.
...
Two things still have to be translated at runtime:

- **Nesting.** The repository JOIN projects flat aliases; `domain.py` still
  builds the nested `Client` tree (`ClientIdentity`, `ComplianceProfile`, …).
- **Figures.** `Client.figures` is assembled from `figure_metrics` rows in a
  separate batched query.

`constants.py` also maps four short aliases (`status`, `risk_level`, `segment`,
...
`hold3_status`. Its dataset import plan fans those into identity + 1:1 satellite
tables and a normalized `figure_metrics` table.

The import plan handles:

- Workbook sheet selection.
- Source column to storage column mappings, for example `client_name` ->
  `clients.name`, `status` -> `case_actions.status`.
...
`account_review/fastmcp/import_plan.py` implements this as ordinary Python:
`TableSchema` constants per table, row splitters for each satellite, and
`_figure_rows` for repeated groups. It inserts clients one at a time so it can
read back each `lastrowid` and attach satellites/figures to the key SQLite
actually assigned.
...
1. The dataset FastMCP server starts from `datasets/<name>/fastmcp/app.py`.
...
- Let AI discovery design SQLite when the workbook shape is not a good storage
  model.
- Prefer SQLite tables aligned to TypeSpec domain groups (or domain-prefixed
  columns when a single table is enough), so the server's mapping stays
  mechanical.
