...
| Engine | Proxy | Notes |
|--------|-------|-------|
| Bing (Azure AI Foundry grounding) | legacy + proxy | Configurable `requires_scraping`; `market` / `setLang` / `freshness` are `ExposableParam` knobs, hidden from the admin form until they are cleared for release |
