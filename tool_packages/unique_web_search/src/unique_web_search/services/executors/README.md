...
- **Search engines** (standard: Google, Brave, Perplexity, Custom API; agent/grounding: Bing, VertexAI)
...
**Search Proxy is transparent to executors:** when `SEARCH_PROXY_BASE_URL` is set, `SearchEngine.search` / `BaseCrawler.crawl` route via the proxy SDK; otherwise they use legacy direct calls. V1 / V2 / V3 do not branch on proxy mode.
...
| **WebSearchV3Executor** | Per-call `search` or `read_urls` | Iterative agent research | Model chooses next cheap search vs full read | One command per tool call | Experimental; loop bounded by orchestrator iteration cap |
